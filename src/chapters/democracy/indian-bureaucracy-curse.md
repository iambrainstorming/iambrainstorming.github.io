# The Iron Cage: Why the Indian Bureaucracy is a Curse on Democracy (And How to Fix It)
...
The goal is to find the optimal subgraph that connects all required nodes with the minimum total weight. 
*Visual Example:* For Student 1 (a1), the algorithm identifies the optimal path: `a1 → b1 (Teacher) → c2 (Content) → d2 (Classroom)` with a total weight of 7.0, which is mathematically better than alternative bureaucratic allocations.
